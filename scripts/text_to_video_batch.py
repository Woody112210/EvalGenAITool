...
PROMPT_FILE = "./prompts/text2video_prompts.csv"
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, f"text_to_video_results_{TIMESTAMP}.csv")
...
def text_to_video(model_path, prompt, extra_args=None):
    ...
    result = handler
    # 提取生成结果链接（假设 result 里有 url 字段或 videos 字段）
...
def main():
    ...
    models = config.get("text_to_video_models", {})
    if not os.environ.get("FAL_KEY"):
        print("请先在 config.yaml 里填写 fal_key！")
        return
    df = pd.read_csv(PROMPT_FILE)
    if PROMPT_COLUMN not in df.columns:
        print(f"文件中未找到 '{PROMPT_COLUMN}' 列！")
        return
    total = len(df)
    results = []
    for idx, row in df.iterrows():
        ...
        for model_name, model_info in models.items():
            ...
            try:
                result_url = text_to_video(model_path, prompt, params)
